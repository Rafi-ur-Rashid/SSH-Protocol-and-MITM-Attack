                     ...
                     unsigned char *buffer, unsigned int *len )
{
        ...
        */
        ...
        memcpy(buffer+TAGSIZE, iv, TAGSIZE);
        memcpy(buffer+(2*TAGSIZE), ciphertext,(int)clen );
        *len=strlen(buffer);
        return 0;
             ...
             unsigned char *plaintext, unsigned int *plaintext_len )
{
    ...
    */
    ...
    int clen=len-2*TAGSIZE;
    int plen=0;
    unsigned char *tag= (unsigned char *)malloc( TAGSIZE );
    unsigned char *iv= (unsigned char *)malloc( TAGSIZE );
    unsigned char *ciphertext= (unsigned char *)malloc( clen );


    memcpy(tag, buffer, TAGSIZE);

    memcpy(iv, buffer+TAGSIZE, TAGSIZE);

    memcpy(ciphertext, buffer+(2*TAGSIZE), clen);


    unsigned char *plaintext = (unsigned char *)malloc( clen+TAGSIZE );
    memset( plaintext, 0, clen+TAGSIZE );

    plen = decrypt( ciphertext, clen, (unsigned char *) NULL, 0,
    tag, key, iv, plaintext );
    memcpy(ptext, plaintext, plen);
...
int seal_symmetric_key( unsigned char *key, unsigned int keylen, EVP_PKEY *pubkey, char *buffer )
{
    ...
    */
    ...
    clen = rsa_encrypt( key, keylen, &ciphertext, &ek, &ekl, &iv, &ivl, pubkey );

    unsigned  char * clen_s=(unsigned char*) malloc(KEYSIZE);
    unsigned  char *ivl_s=(unsigned char*) malloc(KEYSIZE);
    unsigned  char *ekl_s=(unsigned char*) malloc(KEYSIZE);
    memset( clen_s, 0, KEYSIZE );
    memset( ekl_s, 0, KEYSIZE );
    memset( ivl_s, 0, KEYSIZE );


    sprintf(clen_s, "%d", clen);
    ...
    memcpy(buffer, ekl_s, KEYSIZE);
    memcpy(buffer+KEYSIZE, ivl_s, KEYSIZE);
    memcpy(buffer+2*KEYSIZE, clen_s, KEYSIZE);
    memcpy(buffer+3*KEYSIZE, ek, ekl);
    memcpy(buffer+3*KEYSIZE+ekl, iv, ivl);
    memcpy(buffer+3*KEYSIZE+ekl+ivl, ciphertext, clen);
...
int unseal_symmetric_key( char *buffer, unsigned int len, EVP_PKEY *privkey, unsigned char **key)
{
    ...
    */
    ...
    unsigned char ekl_s[KEYSIZE];
    unsigned char ivl_s[KEYSIZE];
    unsigned char clen_s[KEYSIZE];

    memcpy(ekl_s, buffer, KEYSIZE);
    memcpy(ivl_s, buffer+KEYSIZE, KEYSIZE);
    memcpy(clen_s, buffer+2*KEYSIZE, KEYSIZE);
    
   
    unsigned int ekl = (unsigned int)atoi(ekl_s);
    unsigned int ivl=(unsigned int)atoi(ivl_s);
    unsigned int clen=(unsigned int)atoi(clen_s);
    
    unsigned char ek[ekl];
    unsigned char iv[ivl];
    unsigned char ciphertext[clen];

    memcpy(ek, buffer+3*KEYSIZE, ekl);
    memcpy(iv, buffer+3*KEYSIZE+ekl, ivl);
    memcpy(ciphertext, buffer+3*KEYSIZE+ekl+ivl, clen);


    int dlen = rsa_decrypt( ciphertext, clen, ek, ekl, iv, ivl, &plaintext, privkey );
    *key=plaintext;
...
{
    ProtoMessageHdr pmh;
    pmh.msgtype= CLIENT_INIT_EXCHANGE;
    pmh.length=0;
    send_message(sock, &pmh, "");
    
    char block1[MAX_BLOCK_SIZE];
    bzero(block1, MAX_BLOCK_SIZE);
    wait_message( sock, &pmh, block1, SERVER_INIT_RESPONSE);
   
    EVP_PKEY *pubkey;
    unsigned char key[KEYSIZE];
    char block2[MAX_BLOCK_SIZE];

    extract_public_key(block1, pmh.length, &pubkey);
    
    generate_pseudorandom_bytes(key, KEYSIZE);
    bzero(block2, MAX_BLOCK_SIZE);
    
    seal_symmetric_key(key, KEYSIZE, pubkey, block2);
    
    pmh.msgtype=CLIENT_INIT_ACK;
    pmh.length=MAX_BLOCK_SIZE-1;
    send_message(sock, &pmh, block2);

    
    char block3[MAX_BLOCK_SIZE];
    bzero(block3, MAX_BLOCK_SIZE);
    
    wait_message(sock, &pmh, block3, SERVER_INIT_ACK);
    
    unsigned int plen = 0;
    unsigned char *plaintext=(unsigned char*)malloc(MAX_BLOCK_SIZE-1);

    decrypt_message( block3, pmh.length, key, plaintext, &plen);
...
int test_rsa( EVP_PKEY *privkey, EVP_PKEY *pubkey )
{
    ...
    len=seal_symmetric_key( (unsigned char *)"What a victory!", strlen("I am testing code"), pubkey, buffer );
    unseal_symmetric_key( buffer, len, privkey, &key);
...
int test_aes( )
{
    ...
    encrypt_message( msg, len, key, buffer, &buflen );
    decrypt_message( buffer, buflen, key, plaintext, &plen);
...
int server_protocol( int sock, char *pubfile, EVP_PKEY *privkey, unsigned char **enckey )
{
    ...
    */
    ProtoMessageHdr hdr;
    char block1[MAX_BLOCK_SIZE];
    wait_message( sock, &hdr, block1, CLIENT_INIT_EXCHANGE);
    
    

    unsigned char *block2=(unsigned char*) malloc(MAX_BLOCK_SIZE);
    unsigned char* buf;
    unsigned int bytesread=buffer_from_file(pubfile, &buf);
   
    hdr.msgtype= SERVER_INIT_RESPONSE;
    hdr.length=bytesread;
    memcpy(block2, buf, bytesread);
    send_message(sock, &hdr, block2);

    unsigned char *block3=(unsigned char*) malloc(MAX_BLOCK_SIZE);

    wait_message(sock, &hdr, block3, CLIENT_INIT_ACK);
    
    unsigned char *key= (unsigned char*) malloc(KEYSIZE);
    
    unseal_symmetric_key( block3, hdr.length, privkey, &key);   
    *enckey=key;
    
    unsigned char *block4 =(unsigned char*) malloc(MAX_BLOCK_SIZE);
    unsigned int len;
    encrypt_message( (unsigned char*)"Complete", strlen("Complete"), key, block4, &len );
    
    hdr.msgtype= SERVER_INIT_ACK;
    hdr.length=len;

    send_message(sock, &hdr, block4);
...
/**********************************************************************
 ...
 0                 key - the cicpher used to encrypt the traffic
    Outputs     : 0 if successful, -1 if failure
